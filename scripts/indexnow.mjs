...
//
//   INDEXNOW_HOST=client.com INDEXNOW_KEY=<key> npm run indexnow
//
// It reads the live sitemap, extracts every <loc> URL, and submits the
// batch in one request. Generate a key (any 8-128 hex chars), place a
// file at public/<key>.txt containing exactly that key to prove host
// ownership, then pass HOST + KEY via env (per client, never committed).

const HOST = process.env.INDEXNOW_HOST;
const KEY = process.env.INDEXNOW_KEY;
if (!HOST || !KEY) {
  console.error(
    "Set INDEXNOW_HOST and INDEXNOW_KEY env vars. See the comment at the top of this file.",
  );
  process.exit(1);
}
